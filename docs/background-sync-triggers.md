...
| `noteVaultActivityAndScheduleDebounce` | Stamp + arm or mark pending while syncing |
| `scheduleDebouncedSync` / `fireDebouncedSync` | Timer + quiet-window gate; used for vault events and longpoll |
| `flushDeferredAppliesAfterLeafChange` | Immediate apply when unlocked retry-set paths exist |
...
- **Do not put leaf-flush behind vault debounce.** Click-away would wait a full quiet window even though the download was already deferred. Leaf flush must call `syncNow` directly.
