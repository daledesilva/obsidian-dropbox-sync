...
- **Bookkeeping-only success → up to date.** Cycles that only run `recordBase` / mkdir (no upload/download/delete/conflict chips) must show **up to date**, never prose like `78 synced` with an empty non-clickable chip row.
