...
- **Scan fill stays at ~5%.** While a segment is in the scan / “figuring out how many files” phase, the bar holds a small indeterminate stub. Normal left-to-right fill starts once execute progress has a known total.
- **Scan detail never shows vault-hash `N/M`.** Local scan callbacks report vault-wide (and config-disk) hash progress — not the section’s plan. While `phase === "scan"`, the detail line stays **Scanning changes…** until execute seeds real upload/download totals. Do not reuse scan totals as a count link.
...
- **Live execute chips.** Once a section’s plan is ready, upload/download work shows the **same accent chips** as the finished summary, with live **`done / total`** values (accent completed count; theme-normal spaced slash + total). Chips stay clickable from `0 / N` via the live event log (`attempt` / `retrying` / `failed` / `success`). Chip click opens `ActionPathsModal`, which appends events as items progress. On `markResult`, chips settle to count-only values; deletes/conflicts appear only in the finished summary.
- **Bookkeeping-only success → up to date.** Cycles that only run `recordBase` / noop (no upload/download/delete/conflict chips) must show **up to date**, never prose like `78 synced` with an empty non-clickable chip row. Folder creates (`createRemoteFolder` / `createLocalFolder`) **fold into** the normal ↑ upload / ↓ download chips (count + path list), so create+delete fallbacks still show next to trash chips without a separate folder-create icon.
- **R6 deferred → skipped.** Cancel on the upload-ask modal shows **skipped** / **skipped N** (plain text beside any chips), not up to date. See `docs/r6-upload-ask.md`.
...
- **Scan progress must not populate detail counts.** `updateOperationProgress` during `phase === "scan"` zeros `completed`/`total` and keeps **Scanning changes…**; `fillPercent` still forces the 5% stub. Vault-indexed totals (e.g. 96) or config-disk growth (e.g. 110) are misleading for a section chip.
- **Never fall back to `N synced` prose.** `summarizeActions` / `buildSyncResultFeedback` treat empty chip parts as **up to date** so bookkeeping success cannot invent a non-chip summary.
...
- **Live chip totals only for upload/download.** Deletes and conflicts stay finished-only; `LIVE_PROGRESS_ACTION_TYPES` is the allowlist. Folder creates are finished-summary only today (they count under upload/download after execute) — do not invent a separate live folder-create chip.
- **Folder creates share the transfer chip.** `createRemoteFolder` → `upload`, `createLocalFolder` → `download`. Counts and modal path lists mix files and folders for that direction; trash chips still cover `delete*Folder`.
