...
<td>Partially guarded by a per-section ratio heuristic, which is standing in for an explicit scope check. A small or half-present settings tree can still be misread as deletes — and the heuristic only exists for the notes and plugins sections, so settings and workspaces infer deletes with no brake at all. Two further scope defects sit behind this row: A consumes and discards the delta for out-of-scope paths, so it is permanently blind to <code>.obsidian/</code> if that section is ever enabled later (<code>G28</code>), and a delete recorded for an out-of-scope path can never be planned or cleared, which stops A's cursor for every section (<code>G30</code>).</td>
</tr>
</tbody>
</table>
...
| **G30** | Scope the delete log the way planning is scoped — an intent no enabled section can act on freezes everything | 101 | `trackDelete` records every vault delete event that is not exclude-matched, with no scope check at all, and `finalizeState` refuses to commit the cursor while the log is non-empty. Planning is scope-filtered, so an intent for a path outside the current sections is never turned into a `deleteRemote`, never succeeds, and is never cleared — `pruneStaleDeleteLog` keeps it because the path is still in base. Deleting a single `.obsidian/` file on a device whose background sync is notes-only therefore stops the cursor advancing for every section, indefinitely and silently. Same family as `C1` / `C10` / `G27`, but distinct: no bound, retry or user action clears it short of enabling that section or wiping history. |
...
| **G30** | Delete intents are global while planning is scoped, so an unplannable intent stops the cursor | P1 (device stops syncing), P4 (scope is a legitimate per-device choice) | **Yes.** Record intents against the scope that can act on them, or clear any intent no enabled section will ever plan. Must not be "fixed" by planning deletes outside the device's scope — that is exactly what P4 / row 101 forbid. |
...
- **A delete recorded outside the current scope is a permanent brake, not a delayed one.** Delete intents are recorded from vault events with no scope check and gated globally at finalize, while planning is scope-filtered. An intent the enabled sections can never act on is therefore never executed and never cleared, and it stops the cursor for every section (`G30`). Unlike the deferrals above, waiting does not help.
