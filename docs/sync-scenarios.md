...
None open. The Expected Outcomes and P1–P5 / R1–R14 currently agree. Implementation of the former `G*` backlog landed on `release_0.2` (see [Sync gap closure](sync-gap-closure.md)); remaining work is regression coverage and any new gaps found by later audits.

## Gap list

Ordered by how much user data is at risk. Gaps between **the pre-fix codebase** and the **Expected Outcomes** — not between those outcomes and the principles (see [Principles and Rules Gaps](#principles-and-rules-gaps) above). IDs are stable labels rather than positions, so a gap added by a later audit sits where its risk puts it instead of at the end.

**Status (release 0.2):** Gaps **G1–G30** below were closed in commit `86a8714`. Table cells retain the pre-fix audit wording (“Today…”, “Deviates…”) as the historical defect description. Current behaviour is documented in [Sync gap closure](sync-gap-closure.md) and the modules it lists — do not treat this table as a live bug list without re-verifying the code.

Re-audited against the planner, executor, conflict handlers, delete coalescer, engine finalize path, and Dropbox/vault adapters *before* that closure. At audit time every gap already listed still reproduced and none had been fixed. Two entries were wrong or incomplete: `G15` (and row 80) described the wrong failure mode — the Dropbox cursor survives a folder change, so the device keeps reading the *old* folder rather than mass-deleting against the new one — and `G22`'s incomplete-scan brake turns out to cover only the notes and plugins sections. Three gaps were missing entirely. `G28` and `G29` share a root: the plan is built against a *remembered* remote rather than an observed one, and the writes that follow assume that memory is complete. `G30` is unrelated to both — delete intents are recorded and gated globally while planning is scope-filtered, so an intent no enabled section can act on stops the cursor for good.
...
1. **Does Dropbox's move endpoint support a case-only rename?** **Answered (2026-07-26):** No. Official Dropbox API docs for `files/move_v2` and `files/move_batch_v2` state explicitly: *"Note that we do not currently support case-only renaming."* Paths are case-insensitive; a from/to that differs only in casing is treated as the same path. **Implication for G6/G7:** case-only renames need a two-step server move (e.g. `note.md` → `note.md.__case__` → `Note.md`) or an equivalent workaround — a single `move_v2` will not work.

2. **How far back does Dropbox's deletion history reach, and does `list_revisions` still report a deletion after the file itself has aged out?** **Answered (2026-07-26):** Retention matches Dropbox's file recovery window: Basic/Plus/Family **30 days**; Professional/Standard/Essentials **180 days**; Advanced/Enterprise/Education **365 days** (plus extended-history add-ons). `files/list_revisions` returns revisions for a path (or file id); deleted content remains recoverable within that window and can be listed/downloaded by rev while retained. After purge, neither revisions nor recoverability remain — R6's "ask when evidence is missing" applies. Per-path cost: one `list_revisions` call per suspected path (limit default 10, max higher) — batch carefully on fresh joins.

3. **How long is "bounded" for a deferral?** R12 / G10 require every deferral to expire, but the right window is unknown. Long enough that an editor can reload cleanly, short enough that a note left open overnight does not keep the cursor and live polling behind. **Working default for G10 implementation: 60 seconds**, configurable later if needed.

4. **Does a cursor-less `list_folder` with `include_deleted: true` report historical deletions, and for how long?** **Answered (2026-07-26):** Yes within retention. Official docs: with `include_deleted: true`, `list_folder` returns entries for files/folders that used to exist but were deleted, and deleted content may be recovered for as long as revision history retention allows (same windows as Q2). So a cursor-reset full scan **can** clear base-seeded ghosts when Dropbox still reports the tombstone; after retention ages out, absence alone still cannot distinguish delete from never-existed — combine with `list_revisions` (G3) and R6 ask.

**Bonus — upload sessions (G16):** Single `/files/upload` is capped at **150 MB**. Larger files use `/files/upload_session/start` → `append_v2` → `finish`; each request payload ≤ 150 MB; max assembled file **350 GB**; session usable for **48 hours**. Concurrent sessions require 4 MiB-aligned chunks.
